...
///   logger.debug('features.usb_serial.transaction', 'Async listen data: $data');
/// });
///
...
/// if (response_1 == null ) {
///    logger.debug('features.usb_serial.transaction', 'Failed to get a response');
/// }
/// ```
///
class Transaction<T> {
  ...
  static Transaction<Uint8List> terminated(
    ...
    return Transaction<Uint8List>(
      stream.transform(TerminatedTransformer.broadcast(terminator: terminator)),
    );
  }
  ...
  Future<dynamic> getMsg(Duration duration) async {

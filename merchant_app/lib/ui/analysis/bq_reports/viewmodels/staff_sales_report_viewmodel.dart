...
class StaffSalesReportVM extends StoreViewModel with ReportVMBase {
  StaffSalesReportVM.fromStore(Store<AppState> store) : super.fromStore(store);

  List<StaffSalesComparisonReport>? report = [];
  ...
  List<ComparisonBqReportSeries> toAnalysisPairList() {
    return report!.map((compareReport) {
      return ComparisonBqReportSeries(
        ...
        analysisPairs:
            (compareReport.reportResponse as List<BqStaffSalesReport>).map((e) {
              return AnalysisPair(
  ...
  downloadReport(
    ...
    String filePath = await service.downloadStaffSalesReport(
      fileType,
      reportData,
      busDetails,
    );

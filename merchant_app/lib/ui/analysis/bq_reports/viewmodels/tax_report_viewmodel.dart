...
class TaxReportVM extends StoreViewModel with ReportVMBase {
  TaxReportVM.fromStore(Store<AppState> store) : super.fromStore(store);
  ...
  List<ComparisonBqReportSeries> toAnalysisPairList(String reportFilter) {
    return report!.reportDictionary![reportFilter]!.map((compareReport) {
      return ComparisonBqReportSeries(
        ...
        analysisPairs: (compareReport.reportResponse as List<BqTaxReport>).map((
          e,
        ) {
          return AnalysisPair(
  ...
  downloadReport(String fileType, Map<String, List<dynamic>> reportData) async {
    ...
    String filePath = await service.downloadTaxReport(
      fileType,
      reportData,
      busDetails,
    );

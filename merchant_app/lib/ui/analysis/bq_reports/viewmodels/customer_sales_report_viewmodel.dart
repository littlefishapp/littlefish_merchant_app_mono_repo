...
class CustomerSalesReportVM extends StoreViewModel with ReportVMBase {
  CustomerSalesReportVM.fromStore(Store<AppState> store)
    : super.fromStore(store);

  List<CustomerSalesComparisonReport>? report = [];
  ...
  Null loadFromStore(Store<AppState>? store, {BuildContext? context}) {
    ...
    runReport = (ctx) async {
      ...
      await service
          ...
          .whenComplete(() {
            // TODO(lampian): fix error
            isLoading = false;
            if (onReportLoaded != null) {
              onReportLoaded!();
            }
            return report;
          });
  ...
  List<ComparisonBqReportSeries> toAnalysisPairList() {
    return report!.map((compareReport) {
      return ComparisonBqReportSeries(
        ...
        analysisPairs:
            (compareReport.reportResponse as List<BqCustomerSalesReport>).map((
              e,
            ) {
              return AnalysisPair(
                id: e.firstName != null ? e.firstName.toString() : '',
                value: e.amount,
              );
            }).toList(),
      );
  ...
  downloadReport(
    ...
    String filePath = await service.downloadCustomerSalesReport(
      fileType,
      reportData,
      busDetails,
    );

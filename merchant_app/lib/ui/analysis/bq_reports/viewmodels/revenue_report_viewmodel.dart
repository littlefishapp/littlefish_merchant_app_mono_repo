...
class RevenueReportVM extends StoreViewModel with ReportVMBase {
  RevenueReportVM.fromStore(Store<AppState> store) : super.fromStore(store);

  List<AnalysisPair>? paymentTypeReport;
  ...
  loadFromStore(Store<AppState>? store, {BuildContext? context}) {
    ...
    runReport = (ctx) async {
      ...
      report!.reportDictionary!.forEach((key, value) async {
        await service
            .getRevenueReportByFilterCompare(
  ...
  List<ComparisonBqReportSeries> toAnalysisPairList(String reportFilter) {
    return report!.reportDictionary![reportFilter]!.map((compareReport) {
      return ComparisonBqReportSeries(
        ...
        analysisPairs: (compareReport.reportResponse as List<BqRevenueReport>)
            .map((e) {
              return AnalysisPair(
  ...
  downloadReport(String fileType, Map<String, List<dynamic>> reportData) async {
    ...
    String filePath = await service.downloadRevenueReport(
      fileType,
      reportData,
      busDetails,
    );

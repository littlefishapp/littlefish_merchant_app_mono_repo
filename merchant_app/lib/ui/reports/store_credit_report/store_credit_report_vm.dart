...
class StoreCreditReportVM extends StoreViewModel<AppState> with ReportVMBase {
  StoreCreditReportVM.fromStore(Store<AppState> store, {BuildContext? context})
    : super.fromStore(store, context: context);

  List<Customer>? customers;
  List<BusinessUser>? sellers;

  StoreCreditOverview? reportData;
  ...
  loadFromStore(Store<AppState>? store, {BuildContext? context}) {
    ...
    runReport = (ctx) async {
      reportService.getStoreCreditOverview().then((result) {
        reportData = result;
        ...
        if (result == null) {
          ...
          //     builder: (ctxx) => StoreCreditReportPage(
          //           vm: this,
          //         ));
        }
      });
  ...
  Future<StoreCreditOverview> getCustomerReport(context) {
    return reportService.getStoreCreditOverview().then((result) {
      reportData = result;
